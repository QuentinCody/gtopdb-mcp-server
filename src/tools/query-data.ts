...
interface QueryEnv {
    GTOPDB_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("GTOPDB_DATA_DO", "gtopdb");

    server.registerTool(
        "gtopdb_query_data",
        {
            title: "Query Staged GtoPdb Data",
            description:
                "Query staged Guide to PHARMACOLOGY (GtoPdb / IUPHAR) data using SQL. " +
                "Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
